...
public class autopilotService extends Service{
    ...
    private KalmanSimple filterElevater = new KalmanSimple(0.1, 31, 1);
    private KalmanSimple filterAileron = new KalmanSimple(1, 0.1, 1);
    private KalmanSimple filterRudder = new KalmanSimple(0.1, 65, 1);
    
    private KalmanSimple filterThrottle = new KalmanSimple(0.01, 35, 1);
...
//				
//			}
			outThrottle = filterThrottle.update(tmpThrCru + ((r_n_elv * (tmpThrCru*25)) / 100));
			if (Double.isNaN(outThrottle)) {
				outThrottle = 0.0;
			} else if (outThrottle > 1.0) {
				outThrottle = 1.0;
