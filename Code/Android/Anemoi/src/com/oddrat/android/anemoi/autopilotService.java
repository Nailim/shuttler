...
public class autopilotService extends Service{
    ...
    private SensorManager mSensorManager_ori;
    ...
    private static AsyncWriter aw_gpsCSV;
    
    private static AsyncWriter aw_oriCSV;
    
    private static AsyncWriter aw_accCSV;
    private static AsyncWriter aw_magCSV;
    private static AsyncWriter aw_gyrCSV;
    
    private static AsyncWriter aw_aclCSV;
    private static AsyncWriter aw_graCSV;
    private static AsyncWriter aw_rotCSV;
    
    private static AsyncWriter aw_rollPID;
    private static AsyncWriter aw_pitchPID;
    private static AsyncWriter aw_yawPID;
	...
	// garbage collector be damned
	
	// ori
	private static double rudErrRight;
	private static double rudErrLeft;
	
	// gps
	private static Location locNow = new Location("");
	private static Location locWaypoint = new Location("");
	
	private static double distToTarget;
	
	private static double distToNewTarget;
	private static double bearingToNewTarget;
	
	private static double curLatInRad;
	private static double curLonInRad;
	
	private static double newLatInRad;
	private static double dlon;
	private static double newLonInRad;
...
//	
	            ...
	            switch (msg.what) {
	                ...
	                case MSG_SET_SERVICE_STATE:
	                	...
	                	if (isAutopilotServiceRunning) {	// turn on auto pilot
	                		...
	                		// general
	                		try {
	                			aw_gpsCSV = new AsyncWriter(new File(data_session_folder + "/logs/gps.csv"));
	                			aw_gpsCSV.open();
	                			...
	                			aw_oriCSV = new AsyncWriter(new File(data_session_folder + "/logs/ori.csv"));
	                			aw_oriCSV.open();
	                			...
	                			aw_rollPID = new AsyncWriter(new File(data_session_folder + "/logs/pidROLL.csv"));
	                			aw_rollPID.open();
	                			isOpen_file_rollPID = true;	// semaphore
	                			aw_pitchPID = new AsyncWriter(new File(data_session_folder + "/logs/pidPITCH.csv"));
	                			aw_pitchPID.open();
	                			isOpen_file_pitchPID = true;	// semaphore
	                			aw_yawPID = new AsyncWriter(new File(data_session_folder + "/logs/pidYAW.csv"));
	                			aw_yawPID.open();
	                		...
	                		if (settings_autopilot_type == 0) {
	                    		// do android stuff
	                			try {
	                				aw_accCSV = new AsyncWriter(new File(data_session_folder + "/logs/rawACC.csv"));
		                			aw_accCSV.open();
		                			isOpen_file_accCSV = true;	// semaphore
		                			aw_magCSV = new AsyncWriter(new File(data_session_folder + "/logs/rawMAG.csv"));
		                			aw_magCSV.open();
		                			isOpen_file_magCSV = true;	// semaphore
		                			aw_gyrCSV = new AsyncWriter(new File(data_session_folder + "/logs/rawGYR.csv"));
		                			aw_gyrCSV.open();
		                			isOpen_file_gyrCSV = true;	// semaphore
		                			
		                			aw_aclCSV = new AsyncWriter(new File(data_session_folder + "/logs/rawACL.csv"));
		                			aw_aclCSV.open();
		                			isOpen_file_aclCSV = true;	// semaphore
		                			aw_graCSV = new AsyncWriter(new File(data_session_folder + "/logs/rawGRA.csv"));
		                			aw_graCSV.open();
		                			isOpen_file_graCSV = true;	// semaphore
		                			aw_rotCSV = new AsyncWriter(new File(data_session_folder + "/logs/rawROT.csv"));
		                			aw_rotCSV.open();
		                		...
		                		mSensorManager_ori.registerListener(mSensorEventListenerOrientation, mSensorManager.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR), SensorManager.SENSOR_DELAY_GAME);
		                		isActive_mEventListenerOriantation = true;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
    	...
    	if (settings_autopilot_type == 0) {
    		...
    		mSensorManager_ori = (SensorManager) getSystemService(SENSOR_SERVICE);
		...
		minAltitude = 100000.0;	// space and down
    ...
    public void onDestroy() {    	
    	...
    	// old orientation is new rotation
    	if (isActive_mEventListenerOriantation) {
	    	mSensorManager_ori.unregisterListener(mSensorEventListenerOrientation);
    		isActive_mEventListenerOriantation = false;
    	}
    ...
    private Thread gpsFlightGearListener = new Thread() {
		...
		if ((gpsString != null) && (separatedGSPString != null)) {
					...
					aw_gpsCSV.append(Long.toString(currentTime) + "," + separatedGSPString[1] + "," + separatedGSPString[3] + "," + separatedGSPString[5] + "," + "0.0," + separatedGSPString[7] + "," + separatedGSPString[9] + "," + separatedGSPString[11] + "\n");
					//try {
 ...
 // Flightgear ORI listener
    private Thread oriFlightGearListener = new Thread() {
		...
		if ((oriString != null) && (separatedORIString != null)) {			
					...
					aw_oriCSV.append(Long.toString(currentTime) + "," + separatedORIString[1] + "," + separatedORIString[3] + "," + separatedORIString[5] + "\n");
					//try {
					...
					aw_rollPID.append(Long.toString(currentTime) + "," + separatedORIString[5] + "," + r_n_ail + "," + outAileron + "," + e_n_ail + "," + errSUM_ail + "," + dERR_ail + "," + Kp_ail + "," + Ki_ail + "," + Kd_ail + "\n");
					aw_pitchPID.append(Long.toString(currentTime) + "," + separatedORIString[3] + "," + r_n_elv + "," + outElevator + "," + e_n_elv + "," + errSUM_elv + "," + dERR_elv + "," + Kp_elv + "," + Ki_elv + "," + Kd_elv + "\n");
					aw_yawPID.append(Long.toString(currentTime) + "," + separatedORIString[1] + "," + r_n_rud + "," + outRudder + "," + e_n_rud + "," + errSUM_rud + "," + dERR_rud + "," + Kp_rud + "," + Ki_rud + "," + Kd_rud + "\n");
					//try {
...
private Thread AndroidSender = new Thread() {
    ...
    private LocationListener mLocationListener_Start = new LocationListener() {
    		...
    		public void onLocationChanged(Location location) {
				...
				aw_gpsCSV.append(Long.toString(currentTime) + "," + location.getLatitude() + "," + location.getLongitude() + "," + location.getAltitude() + "," + "0.0," + location.getBearing() + "," + location.getSpeed() + "," + location.getAccuracy() + "\n");
    			//try {
		...
		private SensorEventListener mSensorEventListenerOrientation = new SensorEventListener() {        	
			...
			public void onSensorChanged(SensorEvent event) {
				//synchronized (this) {
					...
					try {
						reactSensors((Math.toDegrees(mOrientation[0]) + 180.0), Math.toDegrees(mOrientation[1]), Math.toDegrees(mOrientation[2]), currentTime);
					} catch (Exception e) {
						Log.i("anemoi", "PID exception android (probably nan, ignore)");
					}
					
//	    			// 0 - yaw, 1 - pitch, 2 - roll
					aw_oriCSV.append(Long.toString(currentTime) + "," + (Math.toDegrees(mOrientation[0]) + 180.0) + "," + Math.toDegrees(mOrientation[1]) + "," + Math.toDegrees(mOrientation[2]) + "\n");
					//try {
					...
					aw_rollPID.append(Long.toString(currentTime) + "," + Math.toDegrees(mOrientation[2]) + "," + r_n_ail + "," + outAileron + "," + e_n_ail + "," + errSUM_ail + "," + dERR_ail + "," + Kp_ail + "," + Ki_ail + "," + Kd_ail + "\n");
	    			aw_pitchPID.append(Long.toString(currentTime) + "," + Math.toDegrees(mOrientation[1]) + "," + r_n_elv + "," + outElevator + "," + e_n_elv + "," + errSUM_elv + "," + dERR_elv + "," + Kp_elv + "," + Ki_elv + "," + Kd_elv + "\n");
	    			aw_yawPID.append(Long.toString(currentTime) + "," + (Math.toDegrees(mOrientation[0]) + 180.0) + "," + r_n_rud + "," + outRudder + "," + e_n_rud + "," + errSUM_rud + "," + dERR_rud + "," + Kp_rud + "," + Ki_rud + "," + Kd_rud + "\n");
		...
		private SensorEventListener mSensorEventListenerAccelerometer = new SensorEventListener() {
			...
			public void onSensorChanged(SensorEvent event) {
				//synchronized (this) {
					...
					aw_accCSV.append(Long.toString(currentTime) + "," + event.values[0] + "," + event.values[1] + "," + event.values[2] + "\n");
					//!!!try {
		...
		private SensorEventListener mSensorEventListenerMagnetic = new SensorEventListener() {
			...
			public void onSensorChanged(SensorEvent event) {
				//synchronized (this) {
					...
					aw_magCSV.append(Long.toString(currentTime) + "," + event.values[0] + "," + event.values[1] + "," + event.values[2] + "\n");
					//!!!try {
		...
		private SensorEventListener mSensorEventListenerGyroscope = new SensorEventListener() {
			...
			public void onSensorChanged(SensorEvent event) {
				//synchronized (this) {
					...
					currentTime = System.currentTimeMillis(); //event.timestamp;
	    			// 0 - x, 1 - y, 2 - z
					aw_gyrCSV.append(Long.toString(currentTime) + "," + event.values[0] + "," + event.values[1] + "," + event.values[2] + "\n");
					//!!!try {
		...
		private SensorEventListener mSensorEventListenerGravity = new SensorEventListener() {
			...
			public void onSensorChanged(SensorEvent event) {
				//synchronized (this) {
					...
					aw_graCSV.append(Long.toString(currentTime) + "," + event.values[0] + "," + event.values[1] + "," + event.values[2] + "\n");
					//!!!try {
		...
		private SensorEventListener mSensorEventListenerAcceleration = new SensorEventListener() {
			...
			public void onSensorChanged(SensorEvent event) {
				//synchronized (this) {
					...
					aw_aclCSV.append(Long.toString(currentTime) + "," + event.values[0] + "," + event.values[1] + "," + event.values[2] + "\n");
					//!!!try {
		...
		private void reactGPS(double lat, double lon, double alt, double hdg, double spd, double acc) {
			...
			switch (fpCurrent) {
				...
				case -2:
					...
					distToNewTarget = ((wpCurrent.distAcq / (distToTarget / wpCurrent.distAcq)) / 1000) / 6372.795477598;
					//double distToNewTarget = ((wpCurrent.distAcq) / 1000) / 6372.795477598;
					
					bearingToNewTarget = locWaypoint.bearingTo(locNow) * -1.0;
					...
					curLatInRad = wpCurrent.lat / 180.0 * Math.PI;
					curLonInRad = wpCurrent.lon / 180.0 * Math.PI;
					
					newLatInRad = Math.asin(Math.sin(curLatInRad) * Math.cos(distToNewTarget) + Math.cos(curLatInRad) * Math.sin(distToNewTarget) * Math.cos(bearingToNewTarget));
					dlon = Math.atan2(Math.sin(bearingToNewTarget) * Math.sin(distToNewTarget) * Math.cos(curLatInRad), Math.cos(distToNewTarget) - Math.sin(curLatInRad) * Math.sin(newLatInRad));
					newLonInRad = ((curLonInRad - dlon + Math.PI) % (2*Math.PI)) - Math.PI;
...
//			}
		...
		private void reactSensors(double y_n_rud, double y_n_elv, double y_n_ail, long in_time) {
			...
			rudErrRight = (360.0 - y_n_rud) + r_n_rud;
			rudErrLeft = (-1.0 * y_n_rud) + r_n_rud;
