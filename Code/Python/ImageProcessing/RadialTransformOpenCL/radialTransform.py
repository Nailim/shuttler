...
def processInput() :
	...
	if inputArgs.ppProcess == "bspl" :
		...
		#!print "BEFORE B-TEMPLATE"
		img_beta_f = bSpline.cubic_getBeta(cv_img_roi, img_tpl)
		# Piggyback here
		imgTplY, imgTplX, imgTplParam = img_tpl.shape
		#print img_tpl.shape	# 205, 1292, 2
					# Verjetno je array, od numpy?
		lengthIBF = len(img_beta_f)	
		inBetaMatrika = numpy.zeros(lengthIBF*25, img_beta_f[0][0].dtype)	# ...
		inX = numpy.zeros(lengthIBF, 'int32')	# int32 bo menda vredu?
		inY = numpy.zeros(lengthIBF, 'int32')
		inW = numpy.zeros(lengthIBF, 'int32')
		inH = numpy.zeros(lengthIBF, 'int32')


		counter = 0;
		for i in img_beta_f :
			inX[counter] = img_tpl[i[1]][i[2]][1]
			inY[counter] = img_tpl[i[1]][i[2]][0]
			inW[counter] = i[1];
			inH[counter] = i[2];
			temp = i[0].reshape(25)	# Hardcoded
			for j in range(0, 25):
				inBetaMatrika[counter*25 + j] = temp[j]
			counter =  counter+1

		#!print "AFTER B-TEMPLATE"
	else :
		# default, none
		pass
...
def transformImage(img, img_tpl, img_beta_f = None) :	# img - index of inputArgs.image[]
	...
	if inputArgs.ppProcess == "bspl" :
		...
		#!print "BEFORE B-TRANSFORM"
		cv_img_out = bSpline.cubic_setBeta(cv_img_roi, img_tpl, img_beta_f, inY, inX, inW, inH, inBetaMatrika)
		#!print "AFTER B-TRANSFORM"
	else :
